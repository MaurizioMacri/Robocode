...
public class DamalaTMP extends AdvancedRobot {
	...
	private void collectData(ScannedRobotEvent e) {
		...
		ConvertListToDataset(situations);
...
//		}
	...
	private void doMove() {
		...
		/**
		 * The next section adds a middle point with a random (positive or negative) strength. The strength changes every 5 turns, and goes between -1000 and
		 * 1000. This gives a better overall movement.
		 **/
		midpointcount++;
		if (midpointcount > 5) {
			midpointcount = 0;
		...
		/**
		 * The following four lines add wall avoidance. They will only affect us if the bot is close to the walls due to the force from the walls decreasing at
		 * a power 3.
		 **/
	...
	/**
	 * Turns the shortest angle possible to come to a heading, then returns the direction the the bot needs to move in.
	 **/
...
//		out.println("no situations " + situations.size());
		...
		WaveBullet newWave = new WaveBullet(getX(), getY(), absBearing, firePower, direction, getTime(), newSituation);
		...
		if (situations.size() > 2) {
			...
			if (situations.size() > 700) {
				Dataset data = ConvertListToDataset(situations);
				
				Classifier knn = new KNearestNeighbors(3);
				knn.buildClassifier(data);
//				System.out.println("KNN "+data);
//				newSituation.printEnemySituation();
//				Instance tmpInstance1 =new DenseInstance(newSituation.arrayDouble());
//				String instance = (String) knn.classify(tmpInstance1);
//				best = situations.get(Integer.parseInt(instance));
...
//			out.println("" + angleOffset + "," + direction + "," + guessfactor + "," + best.getMaxEscapeAngle());
	...
	private void robotScanned(ScannedRobotEvent e) {
		...
		// ConvertHashTableToDataset(targets);

	}

	private void doScanner(ScannedRobotEvent e) {
		/**
		 * If i got the target i keep the eyes on it by scanning by WIDTH on both right and left side. It's a perfect lock scan. if e is NULL -> just a 2PI
		 * rotation scan is performed. This metod should be called in run() -> doScanner(null) onScannedRobot() -> doScanner(e) e not NULL
		 */
	...
	public Dataset ConvertListToDataset(List<EnemySituation> list) {
		Dataset tmp = new DefaultDataset();
		for (int i = 0; i < list.size(); i++) {
//			Instance tmpInstance = new DenseInstance(list.get(i).arrayDouble(),Integer.toString(i));
//			tmp.add(tmpInstance);
		}
		return tmp;
	}
